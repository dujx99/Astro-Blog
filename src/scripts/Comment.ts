...
const TwikooFn = async (commentDOM: string) => {
  document.querySelector(commentDOM)!.innerHTML = '<section class="space-loading"><span></span><span></span><span></span></section>'
  await LoadScript("https://cdn.jsdelivr.net/npm/twikoo@1.6.41/dist/twikoo.all.min.js");
  twikoo.init({ envId: SITE_INFO.Comment.Twikoo.envId, el: commentDOM, onCommentLoaded: () => setTimeout(() => document.querySelectorAll('.comment a[href="#"]').forEach(link => link.removeAttribute('href'))) })
}
...
const commentInit = async (key: string, walineInit: any) => {
  // 评论 DOM 
  const commentDOM = '.comment>section'
